import { HttpContextToken, HttpClient, HttpContext } from '@angular/common/http';
...
export class AuthenticationService {
    ...
    validateAccessTokenAndGetAuthorities() {
        return this.http.get(
            'http://localhost:8080/user/current_user_authorities',
            {
    ...
    refreshAccessToken() {
        return this.http.post(
            'http://localhost:8080/user/auth/refresh',
            null,
            {
                withCredentials: true,
                context: new HttpContext().set(SKIP_INTERCEPT, true)
